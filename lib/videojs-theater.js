...
(function(window, vjs) {
  ...
  var theater = function (options) {
    ...
    player.controlBar.children_.forEach(function (children, index) {
      if (children.name_ === 'fullscreenToggle')
        fullScreenIndex = index;
    });

    player.controlBar.el().insertBefore(player.theaterModeToggle.el(), player.controlBar.children_[fullScreenIndex + 1].el());
    return player.theaterModeToggle;
  };
